 ...
 * Shared prompts and utilities used across all vision model providers.
 */
 ...
 * @returns {Object}
 */
export function parseJSON(text, providerName = 'Model') {
  try {
    const match = text.match(/\{[\s\S]*\}/);
    return JSON.parse(match ? match[0] : text);
  } catch {
    throw new Error(`${providerName} returned invalid JSON: ${text}`);
  }
}
